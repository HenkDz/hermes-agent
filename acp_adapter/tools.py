...
def build_tool_title(tool_name: str, args: Dict[str, Any]) -> str:
    ...
    if tool_name == "browser_console":
        ...
        return (
            f"browser console: {expression[:50]}" if expression else "browser console"
        )
...
def _format_browser_result(
    ...
    if tool_name == "browser_console":
        for key in ("result", "output", "value", "logs", "messages"):
            value = data.get(key)
            if value in (None, "", [], {}):
                continue
            rendered = (
            ...
            return _truncate_text(rendered, limit=5000)
        return "Console expression completed"

    if tool_name == "browser_snapshot":
        snapshot = str(
            data.get("snapshot") or data.get("content") or data.get("text") or ""
        ).strip()
        return _truncate_text(snapshot, limit=7000) if snapshot else "Snapshot captured"

    if tool_name == "browser_get_images":
        images = data["images"] if "images" in data else data.get("data")
    ...
    if tool_name == "browser_vision":
        analysis = str(
            data.get("analysis") or data.get("result") or data.get("message") or ""
        ).strip()
        if analysis:
            screenshot_path = str(data.get("screenshot_path") or "").strip()
            lines = [_truncate_text(analysis, limit=6500)]
            if screenshot_path:
                lines.extend(["", f"Screenshot: `{screenshot_path}`"])
            return _truncate_text("\n".join(lines), limit=7000)
        return "Screenshot analyzed"

    if tool_name == "browser_navigate":
        title = str(data.get("title") or data.get("url") or "Page opened").strip()
...
def _format_skills_list_result(result: Optional[str]) -> Optional[str]:
    data = _json_loads_maybe(result)
    if not isinstance(data, dict):
        return result if isinstance(result, str) and result.strip() else None
    if data.get("success") is False or data.get("error"):
        return f"skills_list failed: {data.get('error', 'unknown error')}"
    skills = data.get("skills") or data.get("data") or data.get("items")
    if not isinstance(skills, list):
        return _format_generic_structured_result("skills_list", result)
    lines = [f"Skills available: {len(skills)}"]
    for skill in skills[:20]:
        if isinstance(skill, dict):
            ...
            lines.append(f"- {_truncate_text(str(skill), limit=240)}")
    if len(skills) > 20:
        lines.append(f"... {len(skills) - 20} more")
    return _truncate_text("\n".join(lines), limit=7000)
...
def _build_polished_completion_content(
    ...
    formatter = {
        ...
        "skills_list": lambda: _format_skills_list_result(result),
        ...
        "browser_console": lambda: _format_browser_result(
            tool_name, result, function_args
        ),
...
def build_tool_start(
    ...
    if tool_name == "skills_list":
        ...
        text = "Listing available Hermes skills" + (
    ...
    if tool_name in {
        ...
        if tool_name == "browser_navigate":
            text = f"Opening {arguments.get('url', 'page')}"
        elif tool_name == "browser_snapshot":
            text = None
        elif tool_name == "browser_console":
            expression = str(arguments.get("expression") or "").strip()
            text = "Evaluating browser console expression" + (
                f":\n{_truncate_text(expression, limit=800)}"
                if expression
                else "Reading browser console output"
            )
        elif tool_name == "browser_get_images":
            text = None
        else:
            text = None
        content = [_text(text)] if text else None
        return acp.start_tool_call(
            tool_call_id,
            title,
            kind=kind,
